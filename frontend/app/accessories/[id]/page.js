...
const AccessoriesBooking = ({ params }) => {
    ...
    const fetchAccessoriesDetailsData = async () => {
        try {
            const response = await axios.get(`${process.env.API_URL}/api/accessories/${accessoriesId}/`, {
                headers: headers,
            });
    ...
    const userID = localStorage.getItem('userID');
    const accessoryID = accessoriesId;
    const totalAmount = (accessories && accessories.amount || 0) + deliverycharge;
    const quantity = 1;
    ...
    //console.log(email + '--' + contact + '--' + discount + '--' + deliverycharge + '--' + userID + '--'+ accessoryID + '--' + totalAmount + '--' + quantity + '--' + shippingAddressID + '--' + billingAddressID);

    function makePayment(e, userID, accessoryID, quantity, totalAmount, discount, deliverycharge,shippingAddressID, billingAddressID) {
        e.preventDefault();
                    ...
                    <nav aria-label="breadcrumb">
                        <ol className="breadcrumb">
                            ...
                            <li className="breadcrumb-item active" aria-current="page">
                                <Link href="/accessories">Accessories Booking</Link>
                            </li>
                        </ol>
                    </nav>
                </div>
                        ...
                        {accessories && (
                            <div className="row g-0">
                                <div className="col-md-4 p-3">
                                    <img src={accessories.image} className="img-fluid rounded-start img-thumbnail" alt={accessories.name} />
                                </div>
                                <div className="col-md-8">
                                    <div className="card-body">
                                        <h5 className="card-title">{accessories.name}</h5>
                                        <p className="card-text">{accessories.description}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                            ...
                            <table className="table">
                                <tbody>
                                    <tr>
                                        <td>Price:</td>
                                        <td></td>
                                        <td>Rs.{accessories && accessories.amount.toFixed(2)}</td>
                                    </tr>
                                    <tr>
                                        <td>Discount:</td>
                                        <td></td>
                                    ...
                                    {/* <tr>
                                        ...
                                        <td></td>
                                        <td>Rs. {((accessories && accessories.amount || 0) + deliverycharge + discount).toFixed(2)}</td>
                                    </tr> */}
                                    <tr>
                                        <td className="fw-bold">Total Amount:</td>
                                        <td></td>
                                        <td className="fw-bold">Rs. {((accessories && accessories.amount || 0) + deliverycharge).toFixed(2)}</td>
                                    </tr>
                                </tbody>
                            </table>
                            {(!userAddressData.shipping_addresses || userAddressData.shipping_addresses.length === 0 ||
                                ...
                                <button className="btn btn-primary" onClick={e => { makePayment(e, userID, accessoryID, quantity, totalAmount, discount, deliverycharge, shippingAddressID, billingAddressID) }}>Placed order</button>
                            )}
                        </div>
                    </div>
                </div>
...
export default AccessoriesBooking;
